...
float fast_sigmoid(float x) { return 1.0f / (1.0f + fast_exp(-x)); }

float fast_tanh(float x) { return 2.f / (1.f + fast_exp(-2 * x)) - 1.f; }
...
void Detector::clear() { detector.clear(); }
...
void Detector::load_param(const char *json_file) { return; }
...
void Detector::draw_boxxes(cv::Mat &input, std::vector<BoxInfo> &boxxes) { return; }
