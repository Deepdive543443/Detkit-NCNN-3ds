...
void bordered_resize(ncnn::Mat &src, ncnn::Mat &dst, int dst_w, int draw_coor)
{
    ...
    ncnn::resize_bilinear(src, resized, dst_w, dst_h);
...
void draw_bboxes(const cv::Mat &image, const std::vector<BoxInfo> &bboxes, int v_shift, float x_scaler, float y_scaler)
{
    ...
    for (size_t i = 0; i < bboxes.size(); i++) {
        ...
        ncnn::draw_rectangle_c3(image.data, image.cols, image.rows, (int)x1, (int)y1, (int)w, (int)h, (int)color.rgba,
                                2);

        ncnn::draw_text_c3(image.data, image.cols, image.rows, class_names[bbox.label], (int)x1 + 1, (int)y1 + 1, 7,
                           (int)color.rgba);
    }
    fprintf(stderr, line);
}